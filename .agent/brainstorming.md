...
#### 方案 B：高级多源上传（Advanced Multi-Source Upload）- **Priority: High**

**核心逻辑**：将“题目源”与“作答源”解耦，支持多文件/PDF。

**交互流程设计**：
1.  **步骤一：上传试卷（Question Source）**
    *   支持：多张图片 (JPG/PNG) 或 PDF 文件。
    *   场景：用户上传整张试卷的扫描件或照片。
    *   *AI 预处理*：后台自动将 PDF 转为图片序列，或拼接多张图片。
2.  **步骤二：上传答题卡/草稿纸（Answer Source）**
    *   支持：多张图片。
    *   场景：用户上传仅包含手写答案、红叉、分数的答题卡。
3.  **步骤三：关联与切题（可选项，AI 自动 vs 人工辅助）**
    *   **Level 1 (AI 自动)**：直接把{试卷图集} + {答题卡图集} 全部扔给 Gemini 1.5 Pro，提示：“请在答题卡中找到错题，并根据题号在试卷中找到对应的题干，输出成对的错题分析”。
    *   **Level 2 (人工辅助)**：如果 AI 找不准，允许用户在界面上“圈选”试卷上的题干，再“圈选”答题卡上的错题（交互复杂，暂不推荐作为首选）。

**技术实现路径**：
*   **前端**：改造 `Current Upload` 组件，支持两个 Dropzone（区域 A：试卷；区域 B：答题卡）。
*   **后端**：
    *   Service 接口改造：支持接收 `file[]` 而非单文件。
    *   Prompt 升级：设计 `Multi-Modal Prompt`，明确区分 Image Set A (Context) 和 Image Set B (Target)。
*   **数据存储**：
    *   `questions` 表字段扩展：`source_images` (JSONB array) 存储原始试卷图；`answer_images` (JSONB array) 存储答题痕迹图。

### 决策建议
- **立即执行**：在完成 Case 2（单图模式）测试后，**立即启动方案 B 的开发**。
- **MVP 策略**：先做 Level 1 (AI 自动关联)，利用 Gemini 的长窗口和强推理能力来自动匹配题号。如果效果不佳，再考虑引入人工圈选。
