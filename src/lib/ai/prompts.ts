    ...
    [
      {
        ...
        "primary_knowledge_point": "基于下方的error_analysis，从上述knowledge_points中提取出最能代表本次致命错因的一个核心知识点（纯文本字符串）",
        "error_type": "错误原因分类",
        "error_analysis": "错因深度解析。避免字块（wall of text），请根据逻辑分段。注意：此处的数学变量和公式也必须全部使用 $ $ 包裹。",
        "solution": "标准答案与补救步骤。要求：使用分行展示推导过程，每一行开头尽量使用数学推导符号（如 ∵, ∴, 解：, 又∵）。严禁使用双换行，保持布局紧凑以节省纸张。确保所有数学内容被 $ $ 或 $$ $$ 包裹。",
        "difficulty": 3,
    ...
    [
      {
        ...
        "primary_knowledge_point": "基于下方的error_analysis，从上述knowledge_points中提取出最能代表本次致命错因的一个核心考点（纯文本字符串）",
        "error_type": "知识积累不足 | 理解偏差 | 表述不规范 | 审题缺失",
        "error_analysis": "详细的错因解析。请根据逻辑分段。",
        "solution": "包含【标准答案】与【详细解析】。要求使用列表或短自然段进行结构化编排，避免双换行，保持紧凑。",
        "difficulty": 3,
